...
-- =====================================================================
-- [2026-08-19] scenario_key='8' — 카드 승인/은행 정합성 검증 전용 사용자
--
...
-- '4'/'5'/'6'/'7'(seed-v2-scenario4.sql/5.sql/6.sql/7.sql)에 이어 이 시나리오도 그
-- 구조를 그대로 재사용하는 독립된 사용자(계좌·카드 번호·MOD 계수만 다름)다 — 카드
-- 정합성 로직을 사용자 여러 명으로 병렬 테스트할 때 쓴다. scenario4/6/7 이 그랬던
-- 것처럼 is_large/is_checkcard/small_bc/large_bc/daily_count 의 곱셈 계수를 3/4/5/6/7
-- 어느 것과도 겹치지 않는 새 값으로 골라 날짜별 카테고리·금액·체크/신용 배정이 실제로
-- 다르게 나오도록 했다. 신용카드·체크카드는 정산 방식이 다르므로 소비를 3갈래로
-- 나눠 각기 다른 소스에 기록한다:
--
...
INSERT INTO mock_user (scenario_key, nickname, email) VALUES
  ('8', '카드 정합성 검증 테스트 사용자 8', 'card.split.test8@example.com')
...
SELECT u.id, i.id, '004909******8008', 'KB 카드 정합성 검증 통장', '1001', '01', 'KRW', 3500000.00, 3500000.00, '2026-03-25', '2026-08-31 23:59:00', JSON_OBJECT('scenarioKey', '8')
FROM mock_user u JOIN financial_institution i ON i.institution_code = '0004'
WHERE u.scenario_key = '8'
...
SELECT u.id, i.id, x.no, x.name, x.code, x.type, '01', 'KRW', '2026-03-25', '2026-08-31 23:59:00', JSON_OBJECT('scenarioKey', '8')
FROM mock_user u JOIN financial_institution i ON i.institution_code = '0381'
JOIN (
  SELECT '9490-****-****-8801' AS no, 'KB 카드 정합성 검증 신용카드' AS name, 'KB-CRD-8801' AS code, '01' AS type UNION ALL
  SELECT '5210-****-****-8802', 'KB 카드 정합성 검증 체크카드', 'KB-CHK-8802', '02'
) x
WHERE u.scenario_key = '8'
...
FROM card c JOIN mock_user u ON u.id = c.user_id JOIN bank_account b ON b.user_id = u.id AND b.account_no_masked = '004909******8008'
WHERE u.scenario_key = '8' AND c.card_no_masked IN ('9490-****-****-8801', '5210-****-****-8802')
...
daily_slots AS (
  SELECT d.tx_date, d.day_offset AS n, s.slot_no AS s,
         2 + MOD(MOD(d.day_offset,8) + MOD(d.day_offset,15), 4) AS daily_count,
         (MOD(d.day_offset*37 + s.slot_no*43, 100) < 6) AS is_large,
         (MOD(d.day_offset*19 + s.slot_no*53, 100) < 30) AS is_checkcard
...
computed_rows AS (
  ...
  FROM filtered f
  LEFT JOIN small_bc sc ON sc.idx = MOD(f.n*13 + f.s*37, 17)
  LEFT JOIN large_bc lc ON lc.idx = MOD(f.n*47 + f.s*19, 15)
),
rows_final AS (
  SELECT
...
SELECT
  ...
  CASE WHEN rf.is_checkcard THEN CONCAT('S8-A-CHK-', DATE_FORMAT(rf.tx_date,'%Y%m%d'), '-', LPAD(rf.s,2,'0'))
       ELSE CONCAT('S8-A-CRD-', DATE_FORMAT(rf.tx_date,'%Y%m%d'), '-', LPAD(rf.s,2,'0')) END AS approval_no,
  rf.tat,
  '01', '승인',
  rf.mchv,
  '000-00-00000',
  ...
  JSON_OBJECT(
    ...
    'correlationId', IF(rf.is_checkcard, CONCAT('S8-CHK-', DATE_FORMAT(rf.tx_date,'%Y%m%d'), '-', LPAD(rf.s,2,'0')), NULL),
    'challengeEligible', true
  )
FROM rows_final rf
JOIN mock_user u ON u.scenario_key = '8'
JOIN card chk ON chk.user_id = u.id AND chk.card_no_masked = '5210-****-****-8802'
JOIN card crd ON crd.user_id = u.id AND crd.card_no_masked = '9490-****-****-8801'
...
checkcard_legs AS (
  SELECT
    CONCAT('S8-B-CHK-', DATE_FORMAT(cr.tx_date,'%Y%m%d'), '-', LPAD(cr.s,2,'0')) AS tid,
    TIMESTAMP(cr.tx_date, SEC_TO_TIME(
      (CASE cr.s WHEN 1 THEN 8 WHEN 2 THEN 12 WHEN 3 THEN 15 WHEN 4 THEN 18 ELSE 21 END)*3600
      + MOD(cr.n*7 + cr.s*53, 60)*60
    )) AS tat,
    ...
    CASE WHEN cr.is_large THEN cr.lmcc ELSE cr.smcc END AS mccv,
    CASE WHEN cr.is_large THEN cr.lmch ELSE cr.smch END AS mchv,
    CONCAT('S8-CHK-', DATE_FORMAT(cr.tx_date,'%Y%m%d'), '-', LPAD(cr.s,2,'0')) AS corr
  FROM computed_rows cr
),
ga_categories AS (
...
ga_legs AS (
  SELECT
    CONCAT('S8-A-', DATE_FORMAT(m.month_start,'%Y%m'), '-', LPAD(g.idx,2,'0')) AS tid,
...
settlement_legs AS (
  SELECT
    CONCAT('S8-BILL-', DATE_FORMAT(sm.billing_start,'%Y%m')) AS tid,
    ...
    (SELECT COALESCE(SUM(ca.approved_amount),0) FROM card_approval ca
       ...
       WHERE u.scenario_key = '8' AND crd.card_no_masked = '9490-****-****-8801'
    ...
    CONCAT('S8-CARD-BILL-', DATE_FORMAT(sm.billing_start,'%Y%m')) AS corr
...
FROM all_legs al
JOIN mock_user u ON u.scenario_key = '8'
JOIN bank_account b ON b.user_id = u.id AND b.account_no_masked = '004909******8008'
ON DUPLICATE KEY UPDATE
...
JOIN (
  SELECT DATE('2026-04-01') billing_month, DATE('2026-05-14') due_date, '02' status, '결제완료' status_name, 1 paid_amount_flag, 'S8-CARD-BILL-202604' corr UNION ALL
  SELECT DATE('2026-05-01'), DATE('2026-06-14'), '02', '결제완료', 1, 'S8-CARD-BILL-202605' UNION ALL
  SELECT DATE('2026-06-01'), DATE('2026-07-14'), '02', '결제완료', 1, 'S8-CARD-BILL-202606' UNION ALL
  SELECT DATE('2026-07-01'), DATE('2026-08-14'), '02', '결제완료', 1, 'S8-CARD-BILL-202607' UNION ALL
  SELECT DATE('2026-08-01'), DATE('2026-08-31'), '01', '청구확정', 0, NULL
) x
WHERE u.scenario_key = '8' AND c.card_no_masked = '9490-****-****-8801'
...
WHERE u.scenario_key='8' AND b.account_no_masked='004909******8008';
