...
-- =====================================================================
-- [2026-08-19] scenario_key='6' — 카드 승인/은행 정합성 검증 전용 사용자
--
...
-- 소스를 합쳐서 거래를 감지하는 탕탕 앱의 실제 동작을 재현**했고, scenario_key='4'/'5'
-- (seed-v2-scenario4.sql/seed-v2-scenario5.sql)에 이어 이 시나리오도 그 구조를 그대로
-- 재사용하는 독립된 사용자(계좌·카드 번호·MOD 계수만 다름)다 — 카드 정합성 로직을
-- 사용자 여러 명으로 병렬 테스트할 때 쓴다. scenario5 는 scenario3 의 계수를 그대로
-- 재사용해 소비 패턴이 사실상 동일했으므로, 이 시나리오는 scenario4 가 했던 것처럼
-- is_large/is_checkcard/small_bc/large_bc 의 곱셈 계수를 전부 새 값으로 바꿔 날짜별
-- 카테고리·금액·체크/신용 배정이 3/4/5 와 실제로 다르게 나오도록 했다. 신용카드·
-- 체크카드는 정산 방식이 다르므로 소비를 3갈래로 나눠 각기 다른 소스에 기록한다:
--
...
INSERT INTO mock_user (scenario_key, nickname, email) VALUES
  ('6', '카드 정합성 검증 테스트 사용자 6', 'card.split.test6@example.com')
...
SELECT u.id, i.id, '004909******6006', 'KB 카드 정합성 검증 통장', '1001', '01', 'KRW', 3500000.00, 3500000.00, '2026-03-25', '2026-08-31 23:59:00', JSON_OBJECT('scenarioKey', '6')
FROM mock_user u JOIN financial_institution i ON i.institution_code = '0004'
WHERE u.scenario_key = '6'
...
SELECT u.id, i.id, x.no, x.name, x.code, x.type, '01', 'KRW', '2026-03-25', '2026-08-31 23:59:00', JSON_OBJECT('scenarioKey', '6')
FROM mock_user u JOIN financial_institution i ON i.institution_code = '0381'
JOIN (
  SELECT '9490-****-****-6601' AS no, 'KB 카드 정합성 검증 신용카드' AS name, 'KB-CRD-6601' AS code, '01' AS type UNION ALL
  SELECT '5210-****-****-6602', 'KB 카드 정합성 검증 체크카드', 'KB-CHK-6602', '02'
) x
WHERE u.scenario_key = '6'
...
FROM card c JOIN mock_user u ON u.id = c.user_id JOIN bank_account b ON b.user_id = u.id AND b.account_no_masked = '004909******6006'
WHERE u.scenario_key = '6' AND c.card_no_masked IN ('9490-****-****-6601', '5210-****-****-6602')
...
daily_slots AS (
  SELECT d.tx_date, d.day_offset AS n, s.slot_no AS s,
         ...
         (MOD(d.day_offset*13 + s.slot_no*47, 100) < 6) AS is_large,
         (MOD(d.day_offset*59 + s.slot_no*23, 100) < 30) AS is_checkcard
...
computed_rows AS (
  ...
  FROM filtered f
  LEFT JOIN small_bc sc ON sc.idx = MOD(f.n*29 + f.s*31, 17)
  LEFT JOIN large_bc lc ON lc.idx = MOD(f.n*53 + f.s*41, 15)
),
rows_final AS (
  SELECT
...
SELECT
  ...
  CASE WHEN rf.is_checkcard THEN CONCAT('S6-A-CHK-', DATE_FORMAT(rf.tx_date,'%Y%m%d'), '-', LPAD(rf.s,2,'0'))
       ELSE CONCAT('S6-A-CRD-', DATE_FORMAT(rf.tx_date,'%Y%m%d'), '-', LPAD(rf.s,2,'0')) END AS approval_no,
  rf.tat,
  '01', '승인',
  rf.mchv,
  '000-00-00000',
  ...
  JSON_OBJECT(
    ...
    'correlationId', IF(rf.is_checkcard, CONCAT('S6-CHK-', DATE_FORMAT(rf.tx_date,'%Y%m%d'), '-', LPAD(rf.s,2,'0')), NULL),
    'challengeEligible', true
  )
FROM rows_final rf
JOIN mock_user u ON u.scenario_key = '6'
JOIN card chk ON chk.user_id = u.id AND chk.card_no_masked = '5210-****-****-6602'
JOIN card crd ON crd.user_id = u.id AND crd.card_no_masked = '9490-****-****-6601'
...
checkcard_legs AS (
  SELECT
    CONCAT('S6-B-CHK-', DATE_FORMAT(cr.tx_date,'%Y%m%d'), '-', LPAD(cr.s,2,'0')) AS tid,
    TIMESTAMP(cr.tx_date, SEC_TO_TIME(
      (CASE cr.s WHEN 1 THEN 8 WHEN 2 THEN 12 WHEN 3 THEN 15 WHEN 4 THEN 18 ELSE 21 END)*3600
      + MOD(cr.n*7 + cr.s*53, 60)*60
    )) AS tat,
    ...
    CASE WHEN cr.is_large THEN cr.lmcc ELSE cr.smcc END AS mccv,
    CASE WHEN cr.is_large THEN cr.lmch ELSE cr.smch END AS mchv,
    CONCAT('S6-CHK-', DATE_FORMAT(cr.tx_date,'%Y%m%d'), '-', LPAD(cr.s,2,'0')) AS corr
  FROM computed_rows cr
),
ga_categories AS (
...
ga_legs AS (
  SELECT
    CONCAT('S6-A-', DATE_FORMAT(m.month_start,'%Y%m'), '-', LPAD(g.idx,2,'0')) AS tid,
...
settlement_legs AS (
  SELECT
    CONCAT('S6-BILL-', DATE_FORMAT(sm.billing_start,'%Y%m')) AS tid,
    ...
    (SELECT COALESCE(SUM(ca.approved_amount),0) FROM card_approval ca
       ...
       WHERE u.scenario_key = '6' AND crd.card_no_masked = '9490-****-****-6601'
    ...
    CONCAT('S6-CARD-BILL-', DATE_FORMAT(sm.billing_start,'%Y%m')) AS corr
...
FROM all_legs al
JOIN mock_user u ON u.scenario_key = '6'
JOIN bank_account b ON b.user_id = u.id AND b.account_no_masked = '004909******6006'
ON DUPLICATE KEY UPDATE
...
JOIN (
  SELECT DATE('2026-04-01') billing_month, DATE('2026-05-14') due_date, '02' status, '결제완료' status_name, 1 paid_amount_flag, 'S6-CARD-BILL-202604' corr UNION ALL
  SELECT DATE('2026-05-01'), DATE('2026-06-14'), '02', '결제완료', 1, 'S6-CARD-BILL-202605' UNION ALL
  SELECT DATE('2026-06-01'), DATE('2026-07-14'), '02', '결제완료', 1, 'S6-CARD-BILL-202606' UNION ALL
  SELECT DATE('2026-07-01'), DATE('2026-08-14'), '02', '결제완료', 1, 'S6-CARD-BILL-202607' UNION ALL
  SELECT DATE('2026-08-01'), DATE('2026-08-31'), '01', '청구확정', 0, NULL
) x
WHERE u.scenario_key = '6' AND c.card_no_masked = '9490-****-****-6601'
...
WHERE u.scenario_key='6' AND b.account_no_masked='004909******6006';
