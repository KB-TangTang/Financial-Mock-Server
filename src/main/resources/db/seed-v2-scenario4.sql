...
-- =====================================================================
-- [2026-08-19] scenario_key='4' — 카드 승인/은행 정합성 검증 전용 사용자
--
...
-- 소스를 합쳐서 거래를 감지하는 탕탕 앱의 실제 동작을 재현**했다. 이 시나리오는 같은
-- 구조(그룹 A/B/C 3갈래)를 재사용하지만, MOD 계수를 scenario3 과 다르게 골라 **날짜별
-- 카테고리·금액·체크/신용 배정이 실제로 다르게 나오는 독립된 소비 패턴**을 만든다
-- (처음엔 계좌·카드 번호만 바꿔 scenario3 을 복제했더니 소비 내용까지 숫자 하나 안 틀리고
-- 완전히 동일하게 나오는 문제가 있었음 — MOD 의 입력이 day_offset·슬롯뿐이라 scenario_key
-- 와 무관했기 때문. 계수 자체를 바꿔 해결했다). 신용카드·체크카드는 정산 방식이 다르므로
-- 소비를 3갈래로 나눠 각기 다른 소스에 기록한다:
--
...
INSERT INTO mock_user (scenario_key, nickname, email) VALUES
  ('4', '카드 정합성 검증 테스트 사용자 4', 'card.split.test4@example.com')
...
SELECT u.id, i.id, '004909******4004', 'KB 카드 정합성 검증 통장', '1001', '01', 'KRW', 3500000.00, 3500000.00, '2026-03-25', '2026-08-31 23:59:00', JSON_OBJECT('scenarioKey', '4')
FROM mock_user u JOIN financial_institution i ON i.institution_code = '0004'
WHERE u.scenario_key = '4'
...
SELECT u.id, i.id, x.no, x.name, x.code, x.type, '01', 'KRW', '2026-03-25', '2026-08-31 23:59:00', JSON_OBJECT('scenarioKey', '4')
FROM mock_user u JOIN financial_institution i ON i.institution_code = '0381'
JOIN (
  SELECT '9490-****-****-4401' AS no, 'KB 카드 정합성 검증 신용카드' AS name, 'KB-CRD-4401' AS code, '01' AS type UNION ALL
  SELECT '5210-****-****-4402', 'KB 카드 정합성 검증 체크카드', 'KB-CHK-4402', '02'
) x
WHERE u.scenario_key = '4'
...
FROM card c JOIN mock_user u ON u.id = c.user_id JOIN bank_account b ON b.user_id = u.id AND b.account_no_masked = '004909******4004'
WHERE u.scenario_key = '4' AND c.card_no_masked IN ('9490-****-****-4401', '5210-****-****-4402')
...
daily_slots AS (
  SELECT d.tx_date, d.day_offset AS n, s.slot_no AS s,
         2 + MOD(MOD(d.day_offset,5) + MOD(d.day_offset,13), 4) AS daily_count,
         (MOD(d.day_offset*23 + s.slot_no*19, 100) < 6) AS is_large,
         (MOD(d.day_offset*41 + s.slot_no*7, 100) < 30) AS is_checkcard
...
computed_rows AS (
  ...
  FROM filtered f
  LEFT JOIN small_bc sc ON sc.idx = MOD(f.n*11 + f.s*29, 17)
  LEFT JOIN large_bc lc ON lc.idx = MOD(f.n*43 + f.s*59, 15)
),
rows_final AS (
  SELECT
    ...
    TIMESTAMP(cr.tx_date, SEC_TO_TIME(
      ...
      + MOD(cr.n*13 + cr.s*37, 60)*60
    )) AS tat,
    CASE WHEN cr.is_large THEN
      ROUND((cr.lamin + (cr.lamax-cr.lamin) * MOD(cr.n*47 + cr.s*73, 1000) / 1000) / 100) * 100
    ELSE
      ROUND((cr.samin + (cr.samax-cr.samin) * MOD(cr.n*47 + cr.s*73, 1000) / 1000) / 100) * 100
    END AS amt,
...
SELECT
  ...
  CASE WHEN rf.is_checkcard THEN CONCAT('S4-A-CHK-', DATE_FORMAT(rf.tx_date,'%Y%m%d'), '-', LPAD(rf.s,2,'0'))
       ELSE CONCAT('S4-A-CRD-', DATE_FORMAT(rf.tx_date,'%Y%m%d'), '-', LPAD(rf.s,2,'0')) END AS approval_no,
  rf.tat,
  '01', '승인',
  rf.mchv,
  '000-00-00000',
  ...
  JSON_OBJECT(
    ...
    'correlationId', IF(rf.is_checkcard, CONCAT('S4-CHK-', DATE_FORMAT(rf.tx_date,'%Y%m%d'), '-', LPAD(rf.s,2,'0')), NULL),
    'challengeEligible', true
  )
FROM rows_final rf
JOIN mock_user u ON u.scenario_key = '4'
JOIN card chk ON chk.user_id = u.id AND chk.card_no_masked = '5210-****-****-4402'
JOIN card crd ON crd.user_id = u.id AND crd.card_no_masked = '9490-****-****-4401'
...
checkcard_legs AS (
  SELECT
    CONCAT('S4-B-CHK-', DATE_FORMAT(cr.tx_date,'%Y%m%d'), '-', LPAD(cr.s,2,'0')) AS tid,
    TIMESTAMP(cr.tx_date, SEC_TO_TIME(
      (CASE cr.s WHEN 1 THEN 8 WHEN 2 THEN 12 WHEN 3 THEN 15 WHEN 4 THEN 18 ELSE 21 END)*3600
      + MOD(cr.n*13 + cr.s*37, 60)*60
    )) AS tat,
    ...
    CASE WHEN cr.is_large THEN cr.lmcc ELSE cr.smcc END AS mccv,
    CASE WHEN cr.is_large THEN cr.lmch ELSE cr.smch END AS mchv,
    CONCAT('S4-CHK-', DATE_FORMAT(cr.tx_date,'%Y%m%d'), '-', LPAD(cr.s,2,'0')) AS corr
  FROM computed_rows cr
),
ga_categories AS (
...
ga_legs AS (
  SELECT
    CONCAT('S4-A-', DATE_FORMAT(m.month_start,'%Y%m'), '-', LPAD(g.idx,2,'0')) AS tid,
    TIMESTAMP(DATE_ADD(m.month_start, INTERVAL (3 + MOD(g.idx*11, 25) - 1) DAY), SEC_TO_TIME(9*3600 + MOD(g.idx*17,60)*60)) AS tat,
    '02' AS code, '출금' AS tname,
    ROUND((g.amin + (g.amax-g.amin) * MOD(m.month_offset*29 + g.idx*59, 1000) / 1000) / 100) * 100 AS amt,
    g.mch AS descr, g.cat AS catv, g.mcc AS mccv, g.mch AS mchv, NULL AS corr
  FROM months m CROSS JOIN ga_categories g
),
payday_legs AS (
  SELECT
    CONCAT('S4-SAL-', DATE_FORMAT(m.month_start,'%Y%m')) AS tid,
    TIMESTAMP(DATE_ADD(m.month_start, INTERVAL 24 DAY), SEC_TO_TIME(8*3600 + 30*60)) AS tat,
    '01' AS code, '급여' AS tname,
    2900000 + MOD(m.month_offset*211, 601)*1000 AS amt,
...
settlement_legs AS (
  SELECT
    CONCAT('S4-BILL-', DATE_FORMAT(sm.billing_start,'%Y%m')) AS tid,
    ...
    (SELECT COALESCE(SUM(ca.approved_amount),0) FROM card_approval ca
       ...
       WHERE u.scenario_key = '4' AND crd.card_no_masked = '9490-****-****-4401'
    ...
    CONCAT('S4-CARD-BILL-', DATE_FORMAT(sm.billing_start,'%Y%m')) AS corr
...
FROM all_legs al
JOIN mock_user u ON u.scenario_key = '4'
JOIN bank_account b ON b.user_id = u.id AND b.account_no_masked = '004909******4004'
ON DUPLICATE KEY UPDATE
...
JOIN (
  SELECT DATE('2026-04-01') billing_month, DATE('2026-05-14') due_date, '02' status, '결제완료' status_name, 1 paid_amount_flag, 'S4-CARD-BILL-202604' corr UNION ALL
  SELECT DATE('2026-05-01'), DATE('2026-06-14'), '02', '결제완료', 1, 'S4-CARD-BILL-202605' UNION ALL
  SELECT DATE('2026-06-01'), DATE('2026-07-14'), '02', '결제완료', 1, 'S4-CARD-BILL-202606' UNION ALL
  SELECT DATE('2026-07-01'), DATE('2026-08-14'), '02', '결제완료', 1, 'S4-CARD-BILL-202607' UNION ALL
  SELECT DATE('2026-08-01'), DATE('2026-08-31'), '01', '청구확정', 0, NULL
) x
WHERE u.scenario_key = '4' AND c.card_no_masked = '9490-****-****-4401'
...
WHERE u.scenario_key='4' AND b.account_no_masked='004909******4004';
